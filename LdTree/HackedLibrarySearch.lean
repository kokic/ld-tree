...
namespace Lean.Meta.HackedLibrarySearch
...
def addImport (name : Name) (constInfo : ConstantInfo) :
...
def LibSearchState := IO.Ref (Option (LazyDiscrTree (Name × DeclMod)))

builtin_initialize defaultLibSearchState : IO.Ref (Option (LazyDiscrTree (Name × DeclMod))) ← do
  IO.mkRef .none

instance : Inhabited LibSearchState where
  default := defaultLibSearchState

builtin_initialize ext : EnvExtension LibSearchState ←
...
-/
builtin_initialize abortSpeculationId : InternalExceptionId ←
...
-/
def librarySearchLemma (cfg : ApplyConfig) (act : List MVarId → MetaM (List MVarId))
...
end HackedLibrarySearch

end Lean.Meta.HackedLibrarySearch
